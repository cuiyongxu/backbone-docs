...
var releaseVersion,
	...
	devFile = "dist/jquery.lib",
	minFile = "dist/jquery.min.lib",
	...
	googleFilesCDN = [
		"jquery.lib", "jquery.min.lib", "jquery.min.map"
	],

	msFilesCDN = [
		"jquery-VER.lib", "jquery-VER.min.lib", "jquery-VER.min.map"
	];
	...
	Object.keys( releaseFiles ).forEach(function( key ) {
		...
		if ( !isBeta || key.indexOf( "VER" ) >= 0 ) {
			...
			if ( /\.map$/.test( releaseFile ) ) {
				...
				// "file":"jquery.min.lib","sources":["jquery.lib"]
				text = fs.readFileSync( builtFile, "utf8" )
					.replace( /"file":"([^"]+)","sources":\["([^"]+)"\]/,
						"\"file\":\"" + unpathedFile.replace( /\.min\.map/, ".min.lib" ) +
						"\",\"sources\":[\"" + unpathedFile.replace( /\.min\.map/, ".lib" ) + "\"]" );
