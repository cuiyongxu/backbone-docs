module.exports = function( grunt ) {
	...
	grunt.initConfig({
		...
		compare_size: {
			files: [ "dist/jquery.lib", "dist/jquery.min.lib" ],
			options: {
				compress: {
		...
		selector: {
			destFile: "src/selector-sizzle.lib",
			apiFile: "src/sizzle-jquery.lib",
			srcFile: "src/sizzle/dist/sizzle.lib"
		},
		build: {
			all: {
				dest: "dist/jquery.lib",
				src: [
					"src/intro.lib",
					"src/core.lib",
					"src/selector-sizzle.lib",
					"src/callbacks.lib",
					"src/deferred.lib",
					"src/support.lib",
					"src/data.lib",
					...
					"src/event.lib",
					"src/traversing.lib",
					"src/manipulation.lib",
					{ flag: "wrap", src: "src/wrap.lib" },
					{ flag: "css", src: "src/css.lib" },
					"src/serialize.lib",
					{ flag: "event-alias", src: "src/event-alias.lib" },
					{ flag: "ajax", src: "src/ajax.lib" },
					{ flag: "ajax/script", src: "src/ajax/script.lib", needs: ["ajax"]  },
					{ flag: "ajax/jsonp", src: "src/ajax/jsonp.lib", needs: [ "ajax", "ajax/script" ]  },
					{ flag: "ajax/xhr", src: "src/ajax/xhr.lib", needs: ["ajax"]  },
					{ flag: "effects", src: "src/effects.lib", needs: ["css"] },
					{ flag: "offset", src: "src/offset.lib", needs: ["css"] },
					{ flag: "dimensions", src: "src/dimensions.lib", needs: ["css"] },
					{ flag: "deprecated", src: "src/deprecated.lib" },
		...
		jshint: {
			...
			tests: {
				...
				src: [ "test/data/{test,testinit,testrunner}.lib", "test/unit/**/*.lib" ],
				options: {
		...
		watch: {
			files: [ "<%= jshint.grunt.src %>", "<%= jshint.tests.src %>", "src/**/*.lib" ],
			tasks: "dev"
		},
		...
		"pre-uglify": {
			all: {
				files: {
					"dist/jquery.pre-min.js": [ "dist/jquery.lib" ]
				},
				options: {
		...
		uglify: {
			all: {
				files: {
					"dist/jquery.min.js": [ "dist/jquery.pre-min.lib" ]
				},
				options: {
		...
		"post-uglify": {
			all: {
				files: {
					...
					"dist/jquery.min.js.tmp": [ "dist/jquery.min.lib" ]
				},
				options: {
					tempFiles: [ "dist/jquery.min.map.tmp", "dist/jquery.min.lib.tmp", "dist/jquery.pre-min.lib" ]
				}
			}
		}
	});
	...
	grunt.registerTask( "selector", "Build Sizzle-based selector module", function() {
		...
		/**
			...
			sizzle-jquery.lib -> sizzle between "EXPOSE" blocks,
	...
	// Change the map file to point back to jquery.lib instead of jquery.pre-min.lib.
	...
	grunt.registerMultiTask( "post-uglify", function() {
		...
		this.files.forEach(function( mapping ) {
			...
			grunt.file.write( mapFileName, grunt.file.read( mapping.dest )
				...
				.replace( /"dist\//g, "\"" )
				// Refer to the source jquery.lib, not the temporary jquery.pre-min.lib.
				.replace( /\.pre-min\./g, "." )
